...
def validate(val_loader, model):
    ...
    for images, gts in tqdm(val_loader, desc="Validation"):
        images = images.cuda()
        gts = gts.cuda().long()

        pred = model(images)

        if pred.shape[2:] != gts.shape[1:]:
            pred = F.interpolate(pred, size=gts.shape[1:], mode='bilinear', align_corners=False)

        prob = torch.softmax(pred, dim=1)[:, 1]

        dice = dice_m(prob, gts.float())
        iou = iou_m(prob, gts.float())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_epochs', type=int, default=140, help='epoch number')
    ...
    parser.add_argument('--train_path', type=str, default='./data/train_hist', help='path to train dataset')
    ...
    val_img_paths = all_imgs[num_train:]
    val_mask_paths = all_masks[num_train:]
    ...
    for epoch in range(start_epoch, args.num_epochs + 1):
        ...
        print(f"[Validation] Epoch {epoch} | Dice: {val_dice:.4f} | IoU: {val_iou:.4f}")
